...
contract ZSLMerkleTree {
     ...
     * @return tree root to avoid race condition
     */
    function getWitness(bytes32 cm) public constant returns (uint, bytes32[], bytes32) {
        uint mapIndex = mapCommitmentIndices[cm];
        require(mapIndex > 0);
    ...
    function leftShift(uint256 v, uint256 n) pure public returns (uint256) {
        return v * (2 ** n);
    }

    function rightShift(uint256 v, uint256 n) pure public returns (uint256) {
        return v / (2 ** n);
    }
